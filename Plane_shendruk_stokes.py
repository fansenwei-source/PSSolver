"""Shendruk-Q pilot with complete nematic-stress quasistatic Stokes flow.

This independent script is the second model-migration stage derived from
Plane_fig4_benchmark.py. It implements the Shendruk/Beris--Edwards Q-tensor
equation
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run the Shendruk Q equation coupled to PSSolver's complete "
            "one-constant nematic-stress quasistatic free-slip Stokes solver."
        )
    )
...
class BasisAwareSpectralProjector:
    ...
    This is an exact two-thirds projector for compatible quadratic products.
    The Q molecular field also contains a cubic term. Expanded only in Q, the
    reactive stress is quintic; this script first projects H as a resolved
    field, then evaluates the complete cubic-in-(Q,H) stress without separately
    filtering Q:H. Projecting the complete stress with cubic_half both protects
    the resolved modes and preserves the reactive/alignment energy exchange.
    This is not identical to a monolithic quintic Galerkin convolution or to
    the paper's discretization.
...
class ShendrukQNonlinearModel(torch.nn.Module):
    """Nonlinear part of the Shendruk/Beris--Edwards Q equation.

    The linear molecular-field terms, -A Q/gamma and L1 laplacian(Q)/gamma,
    are advanced implicitly.  This class supplies nonlinear bulk relaxation,
    material advection, full flow alignment, and rigid-body co-rotation.
    """
    ...
    def forward(self, fields, params):
        ...
        # Full Beris--Edwards alignment:
        #
        #   lambda * [E(Q+I/3) + (Q+I/3)E
        #             - 2(Q+I/3)(Q:E)]
        #
        # plus the rigid-body co-rotation Omega Q - Q Omega below.
        lam = ALIGNMENT_PARAMETER
        alignment_xx = lam * (
            (2.0 / 3.0) * Axx
            + 2.0 * (Qxx*Axx + Qxy*Axy + Qxz*Axz)
            - 2.0 * (Qxx + 1.0 / 3.0) * trQA
        )
        alignment_xy = lam * (
            (2.0 / 3.0) * Axy
            + (Qxx*Axy + Qxy*Ayy + Qxz*Ayz)
            + (Axx*Qxy + Axy*Qyy + Axz*Qyz)
            - 2.0 * Qxy * trQA
        )
        alignment_xz = lam * (
            (2.0 / 3.0) * Axz
            + (Qxy*Ayz - Qxz*Ayy + Axy*Qyz - Axz*Qyy)
            - 2.0 * Qxz * trQA
        )
        alignment_yy = lam * (
            (2.0 / 3.0) * Ayy
            + 2.0 * (Qxy*Axy + Qyy*Ayy + Qyz*Ayz)
            - 2.0 * (Qyy + 1.0 / 3.0) * trQA
        )
        alignment_yz = lam * (
            (2.0 / 3.0) * Ayz
            + (Qxy*Axz - Qyz*Axx + Axy*Qxz - Ayz*Qxx)
            - 2.0 * Qyz * trQA
        )
        ...
        out0 = (
            + ldg_b_over_gamma * (
                -(
                    Qxx**2 + Qxy**2 + Qxz**2
                    - 2*Qxx*Qyy - 2*Qyy**2 - 2*Qyz**2
                ) / 3.0
            )
            - ldg_c_over_gamma*Qsq*Qxx
            - ux*gxQxx - uy*gyQxx - uz*gzQxx
            + alignment_xx
            + 2*(wxy*Qxy + wxz*Qxz)
        )
        out1 = (
            - ldg_b_over_gamma*(Qxx*Qxy + Qxy*Qyy + Qxz*Qyz)
            - ldg_c_over_gamma*Qsq*Qxy
            - ux*gxQxy - uy*gyQxy - uz*gzQxy
            + alignment_xy
            + wxy*(Qyy - Qxx) + wxz*Qyz + wyz*Qxz
        )
        out2 = (
            - ldg_b_over_gamma*(Qxy*Qyz - Qxz*Qyy)
            - ldg_c_over_gamma*Qsq*Qxz
        ...
        out3 = (
            + ldg_b_over_gamma * (
                -(
                    -2*Qxx**2 + Qxy**2 - 2*Qxz**2
                    - 2*Qxx*Qyy + Qyy**2 + Qyz**2
                ) / 3.0
            )
            - ldg_c_over_gamma*Qsq*Qyy
            - ux*gxQyy - uy*gyQyy - uz*gzQyy
            + alignment_yy
            - 2*(wxy*Qxy - wyz*Qyz)
        )
        out4 = (
            - ldg_b_over_gamma*(Qxy*Qxz - Qyz*Qxx)
            - ldg_c_over_gamma*Qsq*Qyz
...
class FreeSlipModalSaddleStokesCompute(torch.nn.Module):
    ...
    def _inverse_gradient(
        ...
        gradient_hat, gradient_bcs = self.transform_backend.gradient_hat(
            spectral,
            boundary_conditions,
            axis,
        )
    ...
    def _common_basis_stress_divergence(
        self,
        fields,
        stress_components,
        boundary_conditions,
    ):
        """Differentiate a row-major stress whose components share one basis."""
        stress_hat = fields.transform_tensor(
            torch.stack(stress_components),
            boundary_conditions,
        )
        stress_hat = self.spectral_projector.project(
            stress_hat,
            boundary_conditions,
        )
        derivative_x = self._inverse_gradient(
            fields,
            stress_hat[[0, 3, 6]],
            boundary_conditions,
            axis=0,
        )
        derivative_y = self._inverse_gradient(
            fields,
            stress_hat[[1, 4, 7]],
            boundary_conditions,
            axis=1,
        )
        derivative_z = self._inverse_gradient(
            fields,
            stress_hat[[2, 5, 8]],
            boundary_conditions,
            axis=2,
    ...
    def _distortion_stress_divergence(
        self,
        fields,
        stress_components,
    ):
        """Differentiate distortion stress without mixing DCT and DST parity."""
        ...
        even_hat = self.spectral_projector.project(
            fields.transform_tensor(even_components, Q_BC),
            Q_BC,
        )
        odd_hat = self.spectral_projector.project(
        ...
        even_x = self._inverse_gradient(
            fields,
            even_hat[[0, 2]],
            Q_BC,
            axis=0,
        )
        even_y = self._inverse_gradient(
            fields,
            even_hat[[1, 3]],
            Q_BC,
            axis=1,
        )
        even_z = self._inverse_gradient(
            fields,
            even_hat[4],
            Q_BC,
            axis=2,
        )
        odd_x = self._inverse_gradient(
            fields,
            odd_hat[2],
            DISTORTION_ODD_Z_BC,
            axis=0,
        )
        odd_y = self._inverse_gradient(
            fields,
            odd_hat[3],
            DISTORTION_ODD_Z_BC,
            axis=1,
        )
        odd_z = self._inverse_gradient(
            fields,
            odd_hat[[0, 1]],
            DISTORTION_ODD_Z_BC,
            axis=2,
    ...
    def _compute_nematic_force(self, fields, alpha):
        """Return complete force and its active tangential contribution."""
        q_components = tuple(fields[name] for name in Q_COMPONENTS)
        laplacian_components = tuple(
            fields.laplacian(name)
            for name in Q_COMPONENTS
        )

        # Stress uses raw H, not H/gamma. Treat projected H as a resolved
        # field and do not separately filter Q:H. Projecting only the complete
        # reactive stress preserves its energy-exchange pairing with the Q
        # equation while cubic_half protects the cubic resolved-field product.
        ...
        q_dot_h = q_tensor_contraction(
            q_components,
            h_components,
        )

        reactive_stress = beris_edwards_reactive_stress_components(
            q_components,
            h_components,
        ...
        algebraic_stress = tuple(
            reactive_stress[index]
            + active_prefactor * q_stress[index]
            for index in range(9)
        )
        algebraic_force = self._common_basis_stress_divergence(
            fields,
            algebraic_stress,
            Q_BC,
        )
        ...
        distortion_force = self._distortion_stress_divergence(
            fields,
            distortion_stress,
        )
    ...
    def forward(self, fields, params):
        alpha = params["alpha"]
        force, active_tangential_force = self._compute_nematic_force(
            fields,
            alpha,
        )
        ...
        # Component-wise post-filtering of u would destroy the discrete
        # div(u)=0 pairing.
...
metadata = {
    ...
    "model": {
        ...
        "variant": "shendruk_q_complete_nematic_stress_stokes",
        "stage": "complete_nematic_stress_stokes",
        "Q_convention": Q_convention_metadata(),
        "q_dynamics": {
            "name": "Shendruk/Beris-Edwards",
    ...
    "numerics": {
        "dealiasing": {
            ...
            "nematic_force_evaluation": (
                "project_H_then_each_stress_then_force"
            ),
    ...
    "reproduction_status": "development_complete_nematic_stress_stokes_stage",
...
solver.model.add_dynamic_field(
    "Qxx",
    init = Qxx_0,
    L_hat = -(ldg_a_over_gamma + q2_Q * ldg_l1_over_gamma),
    boundary_conditions = Q_BC,
)
solver.model.add_dynamic_field(
    "Qxy",
...
solver.model.set_nonlinear_model(ShendrukQNonlinearModel(spectral_projector))
