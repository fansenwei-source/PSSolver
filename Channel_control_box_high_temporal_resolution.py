...
external_drive = "/media/fansenwei/Elements"
if not os.path.ismount(external_drive):
    raise RuntimeError(
        f"External drive is not mounted at {external_drive}; refusing to read or "
        "write simulation data on the internal disk."
    )
source_dir = os.path.join(
    external_drive,
    "data_control_box_every_step_dt1e-4",
)
if not os.path.isdir(source_dir):
    raise RuntimeError(f"Resume source directory does not exist: {source_dir}")

snapshot_steps = []
for filename in os.listdir(source_dir):
    if filename.startswith("Q_") and filename.endswith(".npy"):
        step_text = filename[2:-4]
        if step_text.isdigit():
            snapshot_steps.append(int(step_text))
if not snapshot_steps:
    raise RuntimeError(f"No Q snapshots found in resume source: {source_dir}")

start_step = max(snapshot_steps)
if start_step > steps:
    raise RuntimeError(
        f"Latest snapshot step {start_step} exceeds configured final step {steps}"
    )
resume_snapshot_path = os.path.join(source_dir, f"Q_{start_step}.npy")
q_snapshot = load_q_snapshot(
    resume_snapshot_path,
    expected_shape=(Nx, Ny, Nz),
    metadata_directory=source_dir,
    expected_S_bulk=S_BULK,
    require_S_initial=True,
)
q_initial_snapshot = q_snapshot.values
source_run_metadata = q_snapshot.source_metadata
source_S_initial = source_run_metadata.S_initial
output_dir = os.path.join(
    external_drive,
    f"data_control_box_every_step_dt1e-4_resume_from_{start_step}",
)

Qxx_0 = torch.from_numpy(np.ascontiguousarray(q_initial_snapshot[..., 0]))
Qxy_0 = torch.from_numpy(np.ascontiguousarray(q_initial_snapshot[..., 1]))
Qxz_0 = torch.from_numpy(np.ascontiguousarray(q_initial_snapshot[..., 2]))
Qyy_0 = torch.from_numpy(np.ascontiguousarray(q_initial_snapshot[..., 3]))
Qyz_0 = torch.from_numpy(np.ascontiguousarray(q_initial_snapshot[..., 4]))
print(
    f"[q-state continuation] snapshot={resume_snapshot_path} start_step={start_step} "
    f"remaining_steps={steps - start_step}",
    flush=True,
)
...
solver.build()
solver.integrator.restore_progress(start_step)

diagnostic_history = []
run_metadata = {
    "schema_version": 1,
    "script": "Channel_control_box_high_temporal_resolution.py",
    "solver": {
        "shape": [Nx, Ny, Nz],
        "lengths": [Lx, Ly, Lz],
        "dt": dt,
    ...
    "initial_condition": {
        "name": "snapshot",
        "source": resume_snapshot_path,
        "source_step": start_step,
        "mode": "q_state_continuation",
        "source_metadata": str(source_run_metadata.path),
        "source_S_initial": source_S_initial,
        "source_S_bulk": source_run_metadata.S_bulk,
    },
    "continuation_semantics": (
        "Q and control state are restored; quasistatic u, p, and the "
        "iterative pressure guess are recomputed, so this is not a bitwise "
        "exact resume"
    ),
    "continuation_source": {
        "metadata": str(source_run_metadata.path),
        "schema_version": source_run_metadata.metadata.get("schema_version"),
        "script": source_run_metadata.metadata.get("script"),
        "status": source_run_metadata.metadata.get("status"),
    },
}
validate_resume_compatibility(source_run_metadata, run_metadata)
...
source_control_history_path = os.path.join(source_dir, "control_history.csv")
...
if (
    os.path.isfile(source_control_history_path)
    and os.path.getsize(source_control_history_path) > 0
):
    with open(source_control_history_path, newline="") as control_history_file:
        history_rows = list(csv.DictReader(control_history_file))
    eligible_rows = [
        row for row in history_rows
        if row.get("step", "").isdigit() and int(row["step"]) <= start_step
    ]
    if eligible_rows:
        last_control_row = eligible_rows[-1]
        control_state.update({
            "active": last_control_row["control_state"] == "on",
            "above_threshold_count": int(last_control_row["above_threshold_count"]),
            "zero_count": int(last_control_row["zero_count"]),
            "off_checks": int(last_control_row["off_checks"]),
            "last_global_defect_points": int(last_control_row["global_defect_points"]),
            "last_box_defect_points": int(last_control_row["box_defect_points"]),
        })
        if not control_state["active"]:
            alpha.zero_()
        print(
            f"[q-state continuation] control_step={last_control_row['step']} "
            f"control_state={last_control_row['control_state']}",
            flush=True,
        )
with open(control_history_path, "w", newline="") as control_history_file:
    csv.DictWriter(
        control_history_file,
        fieldnames=control_history_fields,
    ).writeheader()
...
start = time.time()
pbar = trange(start_step, steps, initial=start_step, total=steps)
...
# The loop records states before each update (Q_0 through Q_99999). Save the
# state after the final update as well so the complete interval [0, steps] is present.
np.save(f"{output_dir}/Q_{steps}.npy", q_snapshot_numpy(steps))
