...
def validate_task_contract(runs: Sequence[CoreRun], mode: str) -> None:
    ...
    expected_scalar_fields = {
        ...
        "landau_l1": 0.024691358024691357,
        "gamma": 2.94,
        "flow_alignment": 0.3,
    ...
    for run in runs:
        ...
        if metadata.get("diagnostic_interval") != run.artifact.steps:
            errors.append(f"{prefix}: diagnostic_interval must equal completed steps")
        ...
        if metadata.get("boundary_conditions") != expected_boundaries:
            errors.append(f"{prefix}: boundary_conditions do not match the fixed free-slip study")
        ...
        if metadata.get("ldg_coefficients") != {"A": 0.0, "B": -0.3, "C": 0.3}:
            errors.append(f"{prefix}: raw LdG coefficients must be A=0,B=-0.3,C=0.3")

        parameters = metadata.get("model", {}).get("parameters", {})
        for field, expected in expected_model_fields.items():
            if not _metadata_close(parameters.get(field), expected):
                errors.append(f"{prefix}: model.parameters.{field} must be {expected!r}")
        ...
        for hash_name in ("raw_q_sha256", "projected_q_sha256"):
            ...
            if not isinstance(value, str) or len(value) != 64:
                errors.append(f"{prefix}: initial_condition.{hash_name} must be a SHA-256")
...
def physical_scale_summary(runs: Sequence[CoreRun]) -> dict[str, Any]:
    ...
    ldg = metadata["ldg_coefficients"]
    S_bulk = float(metadata["S_bulk"])
    mu_S = (
        float(ldg["A"])
        + float(ldg["B"]) * S_bulk
        + 4.5 * float(ldg["C"]) * S_bulk**2
    )
    landau_l1 = float(metadata["landau_l1"])
...
def weighted_profile_difference(
    ...
    *,
    ...
    numerator = max(
        float(np.trapz(difference * difference * radii, radii)), 0.0
    )
    fine_norm_squared = max(
        float(np.trapz(fine_profile[valid] * fine_profile[valid] * radii, radii)),
        0.0,
    )
    area_weight = float(np.trapz(radii, radii))
    if area_weight <= 0.0:
        return {
            "weighted_difference_rms": math.nan,
            "weighted_relative_l2": math.nan,
...
def time_vs_space_error_gate(
    ...
    identity_mismatches = [
        ...
        if space_fine_input.get(field) != json_safe(time_coarse_identity[field])
    ]
    ...
    return [
        {
            ...
            "raw_q_sha256": run.metadata["initial_condition"]["raw_q_sha256"],
            "projected_q_sha256": run.metadata["initial_condition"][
                "projected_q_sha256"
            ],
...
def write_readme(
    ...
    *,
    ...
    text = f"""# Defect-core spatial-resolution precheck

This directory reports a **Shendruk-inspired A=18 defect-core spatial-resolution
precheck at T=1 under the current PSSolver model**.  It may support a tentative
resolution recommendation for a later long benchmark.  It does not establish
spatial convergence of long-time Fig. 4 dynamics or statistics.

The model differs from the cited paper in four important ways: it uses
quasistatic Stokes rather than a complete momentum equation; the Stokes solve
omits some passive elastic/reactive nematic stresses; `zero_mean` fixes the
free-slip tangential plug mode in a selected reference frame; and the spectral
`cubic_half` discretization differs from the paper's original scheme.  Here
...
def main() -> int:
    ...
    report = {
        ...
        "input_run_manifest": input_run_manifest(runs),
