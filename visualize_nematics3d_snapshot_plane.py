#!/usr/bin/env python3
"""Visualize one nematic snapshot using Nematics3D's analysis and plotting API."""
...
OUTPUT_DIR = Path(__file__).resolve().parent / "data" / "nematics3d_figures"  # Directory where output PNG files are written.
...
PERIODIC_BOUNDARY = (True, True, False)  # Periodic boundary flags for x, y, z during defect detection/classification.
...
DEFECT_FIGURE_SIZE = (2000, 1400)  # Defect figure size in pixels.
...
DEFECT_CAMERA_DISTANCE = 650  # Defect figure camera distance; None keeps Nematics3D's default camera distance.
...
VELOCITY_FIGURE_SIZE = (2000, 1600)  # Velocity figure size in pixels.
...
VELOCITY_CAMERA_DISTANCE = 640  # Velocity figure camera distance; None keeps Nematics3D's default camera distance.
...
VELOCITY_Z_INDEX = 30  # Z slice used for velocity vectors; set to None to use the middle slice.
VELOCITY_STRIDE = 10  # Grid spacing between sampled velocity vectors in x and y.
...
VELOCITY_PLANE_LABEL_POSITION = (0.035, 0.94)  # Viewport coordinates for the velocity plane label.
