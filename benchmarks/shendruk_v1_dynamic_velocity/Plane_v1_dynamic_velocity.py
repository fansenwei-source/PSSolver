...
# Permit direct execution from this benchmark subdirectory without requiring
# an editable PSSolver installation.
...
IMPLEMENTATION_SOURCE_FILES = (
    "benchmarks/shendruk_v1_dynamic_velocity/Plane_v1_dynamic_velocity.py",
    "pssolver/solver.py",
    "pssolver/Field.py",
...
class BerisEdwardsNavierStokesRHS(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # hold the dimensionless dynamic-field projection operator
        #
        #   D = 1 + dt*fric/rho - dt*eta*lap/rho.
        #
        # Its returned pressure multiplier is phi=dt*p/rho.  The integrator
        # converts phi back to physical pressure before storing p.  This avoids
        # allocating a duplicate 3D Schur solver on the GPU.
        self.force_evaluator = BerisEdwardsFreeSlipStokes(
            solver,
            spectral_projector=spectral_projector,
            beta_value=beta_value,
            friction=1.0 + self.dt * self.physical_friction / self.density,
            viscosity=self.dt * viscosity / self.density,
            ldg_a=ldg_a,
            ldg_b=ldg_b,
            ldg_c=ldg_c,
            ldg_l1=ldg_l1,
        ...
        # Backward Euler gives the dimensionless momentum denominator
        # 1 + dt*fric/rho + dt*eta*k^2/rho.  It is strictly positive even at
        # k=0, so the old quasistatic plug-flow null space is absent.
...
class IncompressibleNavierStokesEulerIntegrator(SemiImplicitEulerIntegrator):
    ...
    def __init__(self, model, dt, qx, qy, q2):
        ...
        expected_tangential_inverse = 1.0 / self.denom[5]
        expected_normal_inverse = 1.0 / self.denom[7]
        denominator_rtol = 2.0e-6 if model.dtype == torch.float32 else 1.0e-12
        denominator_atol = 2.0e-7 if model.dtype == torch.float32 else 1.0e-14
        if not torch.allclose(
            self.momentum_solver.a_tangential_inv,
            expected_tangential_inverse,
            rtol=denominator_rtol,
            atol=denominator_atol,
        ) or not torch.allclose(
            self.momentum_solver.a_normal_inv,
            expected_normal_inverse,
            rtol=denominator_rtol,
            atol=denominator_atol,
        ):
            raise RuntimeError(
                "velocity L_hat and incompressible projection denominators disagree"
            )
    ...
    def _cache_momentum_residual(
        ...
        # Convert the dimensionless IMEX residual to physical force-density
        # units.  The exact zero is of course unchanged by this scaling.
        residual_abs = (self.density / self.dt) * residual.abs()
    ...
    def step(self, pre_update_callback=None):
        ...
        # Velocity dynamic fields: the stored L_hat supplies viscosity and
        # physical drag.  Multiplying their ordinary IMEX equation by the
        # denominator gives
        #
        #   D*u^{n+1} + grad(phi^{n+1}) = u^n + dt*N_u(Q^n,u^n),
        #   phi = dt*p/rho.
        #
        # The Schur solve performs this coupled update directly, rather than
        # applying three independent scalar denominators and leaving a
        # compressible tentative velocity.
        old_velocity_hats = fields.spectral[5:8]
        momentum_rhs_hats = old_velocity_hats + self.dt * nonlinear_hats[5:8]
        if self.mean_flow_policy == "zero_mean":
        ...
        ux_hat, uy_hat, uz_hat, pressure_increment_hat = (
        ...
        fields.spectral[self.pressure_index] = (
...
metadata = {
    "schema_version": 1,
    "script": "benchmarks/shendruk_v1_dynamic_velocity/Plane_v1_dynamic_velocity.py",
...
# Velocity uses the same dynamic-field split as Q: viscosity and physical drag
# live in L_hat, while advection and nematic forcing live in the nonlinear RHS.
# The custom integrator applies the resulting denominators together with the
# pressure constraint, because the three components cannot be updated
# independently without violating incompressibility.
zero_velocity = torch.zeros(
    (Nx, Ny, Nz),
    device=device,
    dtype=real_dtype,
)
velocity_tangential_operator = (
    eta / rho * solver.get_laplacian_eigs(U_TANGENTIAL_BC) - fric / rho
)
velocity_normal_operator = (
    eta / rho * solver.get_laplacian_eigs(U_NORMAL_BC) - fric / rho
)
solver.model.add_dynamic_field(
    "ux",
    init=zero_velocity,
    L_hat=velocity_tangential_operator,
...
solver.model.add_dynamic_field(
    "uz",
    init=zero_velocity.clone(),
    L_hat=velocity_normal_operator,
