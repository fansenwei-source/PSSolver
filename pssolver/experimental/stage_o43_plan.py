"""Planning-only contract for the Stage O.4.3 H100 qualification."""
...
def build_stage_o43_h100_plan(
    *,
    ...
    """Return bounded trajectory, profile, and analysis commands."""
    ...
    evidence = Path(stage_o42_report).expanduser().resolve()
    if not evidence.is_file():
        raise FileNotFoundError(f"Stage O.4.2 report is missing: {evidence}")
    if _sha256(evidence) != expected_stage_o42_sha256:
        raise ValueError("Stage O.4.2 report SHA-256 differs from expectation")
    ...
    trajectory_driver = str(
        project / "benchmarks/run_plane_stage_o43_trajectory.py"
    )
    profiler = str(project / "benchmarks/profile_plane_stage_o43.py")
    analyzer = str(
        project / "scripts_plane/analyze_plane_stage_o43_qualification.py"
    )
    trajectory_paths = {
    ...
    analysis.extend(
        (
            ...
            expected_gpu_name,
            "--output",
            str(control / "stage_o43_qualification.json"),
        )
    )
    return {
        "schema_version": 1,
        ...
        "stage_o42_evidence": {
            ...
            "accounting_gate_must_be_closed": True,
        },
        "candidate_contract": {
            "baseline_output_publication": "deferred_stack",
            "baseline_batch_assembly": "copy_cat",
            "candidate_output_publication": "preallocated_packed",
            "candidate_transient_layout": (
        ...
        "stop_conditions": [
            "Git HEAD, clean-worktree, or Stage O.4.2 identity gate fails",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Print the Stage O.4.3 plan")
...
__all__ = ["build_stage_o43_h100_plan", "main"]
