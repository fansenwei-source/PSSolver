"""Bounded Stage N.3 qualification for batched physical computation islands.

The control is the qualified Stage N.2 lazy-materialization runtime.  The
candidate additionally declares physical dependency islands and groups their
projected transforms by boundary signature.  Both remain experimental.
"""
...
def _islands_enabled(mode: str) -> bool:
...
def _build_runtime(
    ...
    *,
    ...
    return build_h100_plane_shadow_runtime_from_production_metadata(
        ...
        enable_batched_physical_islands=_islands_enabled(mode),
    )


def run_stage_n3_h100_candidate_trajectory(
    ...
    *,
    ...
    """Write one short batched-island trajectory from the production Q0."""
    ...
    if reuse is None or reuse.get("retained_pairs_after_generation") != 0:
        raise RuntimeError("Stage N.3 representation lifecycle is invalid")
    if (
        materialization is None
        or materialization.get("physical_materialization_batches", math.inf)
        >= materialization.get("physical_materializations", 0)
        or materialization.get("batched_physical_components", 0) <= 0
    ):
        raise RuntimeError("Stage N.3 did not batch physical computation islands")
    result = {
        "schema_version": 1,
        "qualification_stage": "N.3",
    ...
    _write_new_json(run.output_directory / "stage_n3_metrics.json", result)
    return result
...
def profile_stage_n3_h100_shadow(
    production_directory: str | Path,
    *,
    mode: str,
    warmup_steps: int,
    ...
    """Measure the Stage N.2 control or Stage N.3 candidate on one H100."""

    islands = _islands_enabled(mode)
    ...
    if not finite:
        raise RuntimeError("Stage N.3 profile ended with non-finite fields")
    ...
    return {
        "schema_version": 1,
        "qualification_stage": "N.3",
        "measurement_role": "physical_island_performance_profile",
        "classification": "PROFILE_COMPLETE",
        "mode": mode,
        "algebraic_representation_reuse": True,
        "lazy_algebraic_materialization": True,
        "batched_physical_islands": islands,
        "production_path_changed": False,
        "production_metadata_sha256": reference.metadata_sha256,
        "production_initial_q_sha256": reference.initial_q_file_sha256,
        "configuration": {
...
def analyze_stage_n3_qualification(
    ...
    *,
    ...
    minimum_batched_components: int = 20,
    minimum_maximum_batch_size: int = 5,
    expected_gpu_name: str = "H100",
) -> dict[str, object]:
    """Apply numerical, batching, transform, lifecycle and resource gates."""
    ...
    if (
        ...
        raise ValueError("trajectory comparison violates the fixed N.3 gate")
    ...
    for value, description in (
        (minimum_batched_components, "minimum_batched_components"),
        (minimum_maximum_batch_size, "minimum_maximum_batch_size"),
    ):
        if (
    ...
    for profile, mode in (
        ...
        *((profile, "candidate") for profile in candidates),
        ...
        if (
            ...
            or profile.get("lazy_algebraic_materialization") is not True
            or profile.get("batched_physical_islands")
    ...
    if identities[0] != expected_configuration:
        raise ValueError("profile configuration violates the fixed N.3 gate")
    input_identities = {
        (
    ...
    transform_gate = all(
        ...
        for control, candidate in zip(
            control_forward, candidate_forward, strict=True
        )
    ) and all(
        candidate < control
        for control, candidate in zip(
            control_inverse, candidate_inverse, strict=True
        )
    )
    ...
    control_lifecycle = all(
        ...
        and value.get("physical_materialization_batches")
        == value.get("physical_materializations")
        and value.get("batched_physical_components") == 0
        and profile["transform_call_audit"]["representation_reuse"].get(
            "retained_pairs_after_generation"
        )
        == 0
        for profile, value in zip(controls, control_materializations, strict=True)
    )
    candidate_lifecycle = all(
        isinstance(value, Mapping)
        and value.get("physical_materialization_batches", math.inf)
        < value.get("physical_materializations", 0)
        and value.get("batched_physical_components", 0)
        >= minimum_batched_components
        and value.get("maximum_materialization_batch_size", 0)
        >= minimum_maximum_batch_size
        and value.get("physical_island_prefetches", 0) > 0
        and value.get("on_demand_physical_materializations", math.inf) == 0
        and value.get("unmaterialized_published_components", 0) >= 20
        and profile["transform_call_audit"]["representation_reuse"].get(
            "retained_pairs_after_generation"
        )
        == 0
        for profile, value in zip(
            candidates, candidate_materializations, strict=True
        )
    )
    component_count_gate = all(
        candidate.get("physical_materializations", math.inf)
        <= control.get("physical_materializations", -math.inf)
        for control, candidate in zip(
            control_materializations,
            candidate_materializations,
            strict=True,
    ...
    return {
        ...
        "architecture_decision": "retain_opt_in_batched_physical_islands",
        "eligible_for_stage_n4_architecture_consolidation": accepted,
        ...
        "control": {
            ...
            / len(control_inverse),
            "physical_materialization": control_materializations,
        },
        "candidate": {
        ...
        "comparison": {
            ...
            "transform_call_reduction_gate": transform_gate,
            "control_lifecycle_gate": control_lifecycle,
            "candidate_lifecycle_gate": candidate_lifecycle,
            "component_materialization_count_gate": component_count_gate,
...
def trajectory_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Stage N.3 trajectory")
    ...
    result = run_stage_n3_h100_candidate_trajectory(
...
def profile_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Profile Stage N.3 on H100")
    parser.add_argument("--production-reference-dir", type=Path, required=True)
    ...
    args = parser.parse_args(argv)
    result = profile_stage_n3_h100_shadow(
        args.production_reference_dir,
        mode=args.mode,
...
def analysis_main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Analyze Stage N.3 evidence")
    ...
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)
    result = analyze_stage_n3_qualification(
