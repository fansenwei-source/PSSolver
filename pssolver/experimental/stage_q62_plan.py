"""Planning-only contract for Stage Q.6.2 H100 qualification."""
...
def build_stage_q62_h100_plan(
    *,
    ...
    """Return six balanced profiles, two trajectories, and one analysis."""

    project = _existing_directory(project_root, "project root")
    reference = _existing_directory(
        production_reference, "R320 production reference"
    )
    ...
    profiler = str(project / "benchmarks/profile_plane_stage_q62.py")
    trajectory_runner = str(
        project / "scripts_plane/run_plane_stage_q62_trajectory.py"
    )
    analyzer = str(project / "scripts_plane/analyze_plane_stage_q62.py")
    profile_paths = {
        role: [
            control / "profiles" / f"{role}_trial_{trial}.json"
            for trial in range(1, STAGE_Q62_PROFILE_TRIALS + 1)
        ]
    ...
    trajectory_commands = [
        {
            ...
            "argv": [
                ...
                role,
                "--confirm-steps",
                str(STAGE_Q62_TRAJECTORY_STEPS),
                "--expected-gpu-name",
                expected_gpu_name,
            ],
        }
    ...
    analysis.extend(
        (
            "--stage-q61-report",
            str(q61_path),
            "--expected-stage-q61-sha256",
            expected_stage_q61_sha256,
            "--expected-gpu-name",
            expected_gpu_name,
            "--output",
            str(control / "stage_q62_qualification.json"),
        )
    )
    return {
        "schema_version": 1,
        ...
        "fixed_contract": {
            "shape": [320, 320, 80],
            "profile_trials_per_role": STAGE_Q62_PROFILE_TRIALS,
            "warmup_steps": 10,
            "profile_steps": 20,
            "trajectory_steps": STAGE_Q62_TRAJECTORY_STEPS,
            "trajectory_saved_steps": list(
                range(STAGE_Q62_TRAJECTORY_STEPS + 1)
            ),
            "baseline": "boundary_packed_with_copy_cat_fallback",
            "candidate": "three_source_native_segments",
            "required_sources": [
                "algebraic.nematic_stress.dependencies",
                "explicit_rhs.dependencies",
                "explicit_rhs.outputs",
            ],
            "fallback": "fail_closed_for_designated_sources",
            "changes_equations": False,
            "changes_boundary_signatures": False,
            "changes_transform_partition": True,
        ...
        "stop_conditions": [
            "Git HEAD, worktree cleanliness, or Stage Q.6.1 identity fails",
            ...
            "any designated source uses copy_cat, workspace, or fallback",
            "missing COMPLETE marker, array, profile, or result JSON",
            "trajectory relative-L2 error exceeds 1e-12",
            "peak allocated or reserved ratio exceeds 1.05",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Print the Stage Q.6.2 plan")
    ...
    report = build_stage_q62_h100_plan(
...
__all__ = ["build_stage_q62_h100_plan", "main"]
