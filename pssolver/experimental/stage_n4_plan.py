"""Read-only planning for the bounded Stage N.4 H100 qualification."""
...
def build_stage_n4_h100_plan(
    *,
    project_root: str | Path,
    stage_n3_project_root: str | Path,
    production_reference: str | Path,
    control_root: str | Path,
    scratch_root: str | Path,
    python: str | Path,
    expected_commit: str,
    expected_stage_n3_commit: str,
    expected_gpu_name: str = "H100",
    profile_trials: int = 3,
) -> dict[str, object]:
    """Return the candidate trajectory and balanced N.3/N.4 profiles."""

    project = _existing_directory(project_root, "project root")
    stage_n3_project = _existing_directory(
        stage_n3_project_root,
        "Stage N.3 project root",
    )
    production = _existing_directory(production_reference, "production reference")
    ...
    for value, description in (
        ...
        (expected_stage_n3_commit, "expected_stage_n3_commit"),
    ):
        if (
            ...
            raise ValueError(
                f"{description} must be a full lowercase Git SHA"
            )
    ...
    trajectory = [
        python_text,
        str(project / "Plane_beris_edwards_shadow_stage_n4.py"),
    ...
    for trial in range(1, profile_trials + 1):
        ...
        commands = {
            "control": [
                ...
                str(
                    stage_n3_project
                    / "benchmarks/profile_plane_shadow_stage_n3.py"
                ),
                "--production-reference-dir",
                str(production),
                "--mode",
        ...
        profile_commands.extend(
            {"trial": trial, "mode": mode, "argv": commands[mode]}
            for mode in order
        )
    analysis = [
        python_text,
        str(project / "scripts_plane/analyze_plane_shadow_stage_n4.py"),
    ...
    analysis.extend(
        (
            ...
            expected_gpu_name,
            "--output",
            str(control / "stage_n4_qualification.json"),
        )
    )
    return {
        "schema_version": 1,
        ...
        "control_definition": "frozen_stage_n3_parent_commit",
        "candidate_definition": "unified_execution_policy_and_scheduler",
        "production_path_changed": False,
        "production_promotion_authorized": False,
        "long_simulation_authorized": False,
        "paths": {
            "project_root": str(project),
            "stage_n3_project_root": str(stage_n3_project),
        ...
        "fixed_gates": {
            ...
            "materialization_lifecycle_must_match": True,
            "profile_final_q_sha256_must_match": True,
            "representation_cache_must_retain_no_tensor_pairs": True,
        },
        "commands": {
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Print the Stage N.4 H100 plan")
    parser.add_argument("--project-root", type=Path, required=True)
    parser.add_argument("--stage-n3-project-root", type=Path, required=True)
    parser.add_argument("--production-reference", type=Path, required=True)
    parser.add_argument("--control-root", type=Path, required=True)
    parser.add_argument("--scratch-root", type=Path, required=True)
    parser.add_argument("--python", type=Path, required=True)
    parser.add_argument("--expected-commit", required=True)
    parser.add_argument("--expected-stage-n3-commit", required=True)
    ...
    plan = build_stage_n4_h100_plan(
        ...
        stage_n3_project_root=args.stage_n3_project_root,
        production_reference=args.production_reference,
        control_root=args.control_root,
        scratch_root=args.scratch_root,
        python=args.python,
        expected_commit=args.expected_commit,
        expected_stage_n3_commit=args.expected_stage_n3_commit,
...
__all__ = ["build_stage_n4_h100_plan", "main"]
