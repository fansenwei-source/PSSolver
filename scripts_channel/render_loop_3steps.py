...
def make_contact_sheet(
    ...
    try:
        ...
        axis_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 34)
    except Exception:
        axis_font = None
    axis_y = 48
    left_x = thumb_size // 2
    right_x = width - thumb_size // 2
    draw.line((left_x, axis_y, right_x, axis_y), fill=(30, 30, 30), width=5)
    arrow = 20
    draw.polygon(
        [
            (right_x, axis_y),
    ...
    draw.text(
        ((left_x + right_x - (bbox[2] - bbox[0])) / 2, 7),
        label,
        fill=(0, 0, 0),
        font=axis_font,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPO_ROOT / "data_control_box" / "loop_summary" / "loop_3steps.png",
    )
...
def main() -> None:
    ...
    for step in args.steps:
        ...
        q5 = load_q_snapshot(q_path, require_S_initial=True).values
    ...
    make_contact_sheet(image_paths, infos, args.output)
