...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--save-beta-csv",
        action="store_true",
        help="Calculate real beta and save sampled line data as CSV.",
    )
    parser.add_argument(
        "--beta-csv",
        type=Path,
        default=None,
        help="CSV output path; defaults to the image name with a .csv suffix.",
    )
...
def load_q(data_dir: Path, step: int) -> np.ndarray:
    ...
    q5 = load_q_snapshot(path, require_S_initial=True).values
...
def smooth_or_raw(
    ...
    if getattr(line, "calc_defect_num", 0) >= 5:
        try:
            # Classified loops end on neighboring grid points.  Periodic
            # smoothing removes the otherwise visible seam at that join.
            is_closed = np.linalg.norm(raw_coords[0] - raw_coords[-1]) <= np.sqrt(3.0)
            smooth = line.act_smooth(
                window_length=int(window_length),
                num_out_ratio=float(density),
                min_line_length=5,
                mode="wrap" if is_closed else "interp",
...
def real_beta_for_line(
    ...
    samples_count: int,
    csv_rows: list[tuple[float, ...]] | None = None,
) -> np.ndarray | None:
    if len(coords) < 5:
        return None
        ...
        if np.isfinite(beta):
            ...
            if csv_rows is not None:
                try:
                    ...
                    tangent, _ = beta_source.act_calc_tangent(
                        float(sample),
...
def save_beta_csv(path: Path, rows: list[tuple[float, ...]]) -> None:
    if not rows:
        raise RuntimeError("real beta was not available for CSV output")
    header = (
        "u_percent,beta_rad,beta_deg,x,y,z,"
        "omega_x,omega_y,omega_z,tangent_x,tangent_y,tangent_z"
    )
...
def main() -> None:
    ...
    for line in q_obj.lines:
        coords, smooth = smooth_or_raw(
            line,
            density=float(args.line_smooth_density),
            window_length=int(args.line_smooth_window_length),
        )
    ...
    plotted = 0
    plotted_real_beta = 0
    beta_csv_rows: list[tuple[float, ...]] = []
    calculate_real_beta = bool(args.real_beta or args.save_beta_csv)
    for line, coords, smooth in line_items:
        ...
        beta = (
            real_beta_for_line(
                ...
                coords,
                smooth,
                int(args.beta_samples),
                csv_rows=(
                    beta_csv_rows
                    if args.save_beta_csv and not beta_csv_rows
                    else None
                ),
            )
            if calculate_real_beta
            else None
        )
    ...
    if args.save_beta_csv:
        beta_csv_path = args.beta_csv or args.output.with_suffix(".csv")
        save_beta_csv(beta_csv_path, beta_csv_rows)
        print(f"beta_csv={beta_csv_path} samples={len(beta_csv_rows)}")
    print(f"output={args.output}")
    print(f"lines_plotted={plotted}")
    if calculate_real_beta and plotted_real_beta == plotted and plotted > 0:
        print("beta_mode=real")
    elif calculate_real_beta and plotted_real_beta > 0:
