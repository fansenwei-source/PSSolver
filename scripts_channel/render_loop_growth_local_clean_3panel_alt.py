...
def main() -> None:
    ...
    for index, (q5, info) in enumerate(zip(q_snapshots, infos)):
        image_path = args.output_dir / f"loop_local_alt_step_{info.step:05d}.png"
        render_loop(
            image_path,
            q5,
            info,
    ...
    write_manifest(args.output_dir / "loop_growth_3panel_alt_manifest.csv", infos, image_paths)
    contact = args.output_dir / "loop_growth_3panel_alt.png"
    make_contact_sheet(image_paths, infos, contact)
    print(f"manifest={args.output_dir / 'loop_growth_3panel_alt_manifest.csv'}")
