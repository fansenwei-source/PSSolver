...
def parse_args() -> argparse.Namespace:
    default_dir = REPO_ROOT / "data_control_box" / "loop_summary"
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--five-panel",
        type=Path,
        default=default_dir / "loop_3steps.png",
    )
    parser.add_argument(
        "--geometry",
        type=Path,
        default=default_dir / "alpha_geometry.png",
    )
    parser.add_argument(
        "--radius",
        type=Path,
        default=default_dir / "loop_radius.png",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=default_dir / "loop_summary.png",
    )
