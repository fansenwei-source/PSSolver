...
DEFAULT_STEPS = (6100, 6200, 6400, 6800, 7100)
...
def render_loop(
    ...
    coords, orient, scalars = sample_local_directors(
        q5,
        box.center,
...
def make_contact_sheet(
    ...
    try:
        ...
        axis_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 18)
        tick_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 17)
    except Exception:
        axis_font = None
        tick_font = None
    axis_y = 38
    ...
    draw.line((left_x, axis_y, right_x, axis_y), fill=(30, 30, 30), width=3)
    arrow = 10
    draw.polygon(
        [
            (right_x, axis_y),
    ...
    draw.text((left_x, 7), "time step", fill=(0, 0, 0), font=axis_font)
    for idx, info in enumerate(infos):
        x = idx * thumb_size + thumb_size // 2
        draw.line((x, axis_y - 8, x, axis_y + 8), fill=(30, 30, 30), width=2)
        label = str(info.step)
        bbox = draw.textbbox((0, 0), label, font=tick_font)
        draw.text(
            (x - (bbox[2] - bbox[0]) / 2, axis_y + 14),
            label,
            fill=(0, 0, 0),
            font=tick_font,
        )
