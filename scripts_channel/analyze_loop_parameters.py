...
3. Construct the local ``{N, M, L}`` frame
   A self-consistent principal-plane calculation samples the local Q field,
   rebuilds unit-order Q tensors from the sampled directors, averages them,
   and diagonalizes the average.  The resulting orthonormal axes are ``N``
   (background director), ``M`` (principal perturbation), and ``L`` (normal of
   the N-M principal plane).
...
def load_q(path: Path) -> np.ndarray:
    ...
    if q5.ndim != 4 or q5.shape[-1] != 5:
        raise ValueError(
            f"{path} has shape {q5.shape}; expected component-last (Nx, Ny, Nz, 5)"
        )
    return np.asarray(q5)


def build_q_object(q5: np.ndarray, step: int) -> n3d.QFieldObject:
    return n3d.QFieldObject(
...
def fit_wedge_profile(
    ...
    *,
    ...
    _, directors = n3d.Q_diagonalize(q_values)
...
def measure_dominant_k(
    ...
    *,
    ...
    component_angles = {
        "splay_axis_N_deg": unsigned_angle_deg(k_hat, axis_n),
        "bend_axis_M_deg": unsigned_angle_deg(k_hat, axis_m),
        "twist_axis_L_deg": unsigned_angle_deg(k_hat, axis_l),
    }
    nearest_type = min(component_angles, key=component_angles.get).split("_axis", 1)[0]
